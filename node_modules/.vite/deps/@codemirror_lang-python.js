import { E as syntaxTree, St as Tree, _ as foldNodeProp, _t as NodeProp, b as indentNodeProp, bt as NodeWeakMap, gt as IterMode, h as foldInside, ht as DefaultBufferLength, i as completeFromList, l as LRLanguage, mt as tags, o as ifNotIn, p as delimitedIndent, pt as styleTags, s as snippetCompletion, u as LanguageSupport, vt as NodeSet, xt as Parser, yt as NodeType } from "./dist-Ctla5ozN.js";
...
var newline = 10, carriageReturn = 13, space = 32, tab = 9, hash = 35, parenOpen = 40, dot = 46, braceOpen = 123, braceClose = 125, singleQuote = 39, doubleQuote = 34, backslash = 92, letter_o = 111, letter_x = 120, letter_N = 78, letter_u = 117, letter_U = 85;
...
var indentation = new ExternalTokenizer((input, stack) => {
	...
	if (prev == newline || prev == carriageReturn) {
		...
		if (depth != context.indent && input.next != newline && input.next != carriageReturn && input.next != hash) if (depth < context.indent) input.acceptToken(dedent, -chars);
		else input.acceptToken(indent);
	}
});
var cx_Bracketed = 1, cx_String = 2, cx_DoubleQuote = 4, cx_Long = 8, cx_Raw = 16, cx_Format = 32;
...
var strings = new ExternalTokenizer((input, stack) => {
	...
	for (;;) if (input.next < 0) break;
		...
		break;
	} else if (input.next == backslash && !escapes && input.peek(1) > -1) input.advance(2);
...
//#endregion
//#region node_modules/@codemirror/lang-python/dist/index.js
...
var gatherCompletions = {
	...
	ForStatement(node, def, outer) {
		if (outer) {
			for (let child = node.node.firstChild; child; child = child.nextSibling) if (child.name == "VariableName") def(child, "variable");
...
var pythonLanguage = /* @__PURE__ */ LRLanguage.define({
	...
	parser: /* @__PURE__ */ parser.configure({ props: [/* @__PURE__ */ indentNodeProp.add({
		...
		MatchBody: (context) => {
			var _a;
			return (_a = indentBody(context, innerBody(context) || context.node)) !== null && _a !== void 0 ? _a : context.continue();
		},
...
*/
function python() {
	return new LanguageSupport(pythonLanguage, [pythonLanguage.data.of({ autocomplete: localCompletionSource }), pythonLanguage.data.of({ autocomplete: globalCompletion })]);
}
//#endregion
