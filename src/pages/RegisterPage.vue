  ...
  <div class="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
    ...
    <main class="flex-1 flex items-center justify-center px-4 py-12">
      <section class="w-full max-w-md rounded-2xl bg-white border border-slate-200 shadow-xl p-8">
        <h2 class="text-2xl font-bold text-slate-800 mb-2">注册</h2>
        <p class="text-sm text-slate-500 mb-6">创建账号后即可登录做实验。</p>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          <label class="block">
            <span class="mb-1 block text-sm font-medium text-slate-600">姓名</span>
            <input v-model="form.name" class="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500" type="text" />
          </label>
          <label class="block">
        ...
        <button class="mt-4 w-full rounded-lg border border-blue-200 px-4 py-3 font-semibold text-blue-600 hover:bg-blue-50" type="button" @click="router.push('/login')">
          去登录
        </button>
      </section>
    </main>
