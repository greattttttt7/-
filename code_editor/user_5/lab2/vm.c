...
pte_t *walk(pagetable_t pagetable, uint64 va, int alloc)
{
	...
	// ==========================================================
	// TODO (Lab2 - Task 7): 实现页表遍历
	// ------------------------------------------------------------
	// 目标: 遍历 Sv39 三级页表，找到虚拟地址 va 对应的 PTE 地址。
	// 提示:
	//   1. 从 level=2 开始，向下遍历到 level=1。
	//   2. 使用 PX(level, va) 获取当前级别的页表索引。
	//   3. 如果 PTE 有效(PTE_V)，则获取下一级页表地址(PTE2PA)。
	//   4. 如果 PTE 无效且 alloc=1，则分配新的页表页。
	//   5. 最后返回 level=0 的 PTE 地址 &pagetable[PX(0, va)]。
	// ==========================================================
	//*************************Your code here.****************************
...
int mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
	...
	// ==========================================================
	// TODO (Lab2 - Task 8): 实现页表映射创建
	// ------------------------------------------------------------
	// 目标: 创建虚拟地址 va 到物理地址 pa 的映射，长度为 size。
	// 提示:
	//   1. 使用 walk() 函数查找或创建 PTE（alloc=1）。
	//   2. 如果 PTE 已经存在(PTE_V)，说明重复映射，返回 -1。
	//   3. 设置 PTE: *pte = PA2PTE(pa) | perm | PTE_V。
	//   4. 每次循环后，va 和 pa 都增加 PGSIZE。
	//   5. 当 a == last 时退出循环。
	// ==========================================================
	//*************************Your code here.****************************
...
{
	// ==========================================================
	// TODO (Lab2 - Task 9): 实现页表递归释放
	// ------------------------------------------------------------
	// 目标: 递归释放三级页表的所有页表页。
	// 提示:
	//   1. 遍历页表中的所有 512 个 PTE。
	//   2. 如果 PTE 有效且不是叶子节点(无 R/W/X 权限)：
	//      - 获取子页表的物理地址(PTE2PA)。
	//      - 递归调用 freewalk 释放子页表。
	//      - 将当前 PTE 清零。
	//   3. 如果 PTE 有效但却是叶子节点，说明出错，panic。
	//   4. 最后调用 kfree 释放当前页表页。
	// ==========================================================
	//*************************Your code here.****************************
